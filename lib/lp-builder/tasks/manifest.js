...
function writeLangpackManifest(lpBuilder, files) {
  ...
  lpBuilder.config.LOCALES.forEach(function(locale) {
    ...
    var speechDataSrcDir = path.join(lpBuilder.config.LOCALE_BASEDIR, 'speech-data');
    if (utils.fileExists(speechDataSrcDir)) {
       ...
       speechDataFilePaths.forEach(function(speechDataFilePath) {
         speechDataEntries.push("/" + locale + "/speech-data/" + speechDataFilePath.replace(/^.*[\\\/]/, ''));
       });
       manifest['languages-provided'][locale]["speech-data"] = speechDataEntries;
    }
