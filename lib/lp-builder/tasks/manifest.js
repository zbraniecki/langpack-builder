...
function writeLangpackManifest(lpBuilder, files) {
  brandAST = Parser.parse('l10n.js', files[1]);
  var descAST = Parser.parse('l10n.js', files[0]);
  var manifest = {};

  manifest.name = getEntryFromAST(descAST, 'langpack_english', 'name');
  manifest.description = getEntryFromAST(
    descAST, 'langpack_english', 'description');
  ...
  lpBuilder.config.LOCALES.forEach(function(locale) {
    ...
    var speechDataSrcDir = path.join(
      lpBuilder.config.LOCALE_BASEDIR, 'speech-data');
    if (utils.fileExists(speechDataSrcDir)) {
       var speechDataFilePaths = utils.ls(speechDataSrcDir, false, /.*/);
       var speechDataEntries = [];
       speechDataFilePaths.forEach(function(speechDataFilePath) {
         speechDataEntries.push(
           '/' + locale + '/speech-data/' +
           speechDataFilePath.replace(/^.*[\\\/]/, ''));
       });
       manifest['languages-provided'][locale]['speech-data'] =
         speechDataEntries;
    }
  ...
  var manifestPath = path.join(
    lpBuilder.config.LP_RESULT_DIR, 'manifest.webapp');
...
function copyIcon(config) {
  ...
  var newFile = fs.createWriteStream(
    path.join(config.LP_RESULT_DIR, './icon.png'));
...
var addLangpackManifest = function(lpBuilder) {
  var descPath = path.join(
    lpBuilder.config.LOCALE_BASEDIR, 'shared', 'langpack', 'desc.properties');
  var brandPath = path.join(
    lpBuilder.config.GAIA_DIR, 'shared', 'locales', 'branding', 'official',
    'branding.en-US.properties');
  ...
  return Promise.all(getFiles).then().then(
    writeLangpackManifest.bind(null, lpBuilder)).then(
    copyIcon.bind(null, lpBuilder.config));
};
