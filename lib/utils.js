  ...
  return exports.getFileContent(htmlPath)
    .then(function(content) {
      if (content.indexOf('localization') === -1) {
        ...
        return exports.getDocument(content).then(function($) {
...
exports.getAppDirs = function(config) {
  ...
  return exports.getFileContent(listPath).then(function(content) {
    var apps = {};
    content.split('\n').forEach(function(line) {
      ...
      if (matched) {
        if (matched[2] === '*') {
          ...
          if (exports.fileExists(p)) {
            var dirs = exports.getDirs(p);
          }
        } else {
          if (apps[matched[2]]) {
            throw new Error('two apps with same name: \n  - ' + line);
          }
          if (exports.fileExists(Path.join(config.GAIA_DIR, line))) {
            apps[matched[2]] = line;
          }
        }
      } else if (line) {
...
function flattenResources(gaiaPath, resLists) {
  ...
  resLists.forEach(function(resList) {
    resList.forEach(function(resPath) {
      var fullResPath = exports.buildResourcePath(resPath, gaiaPath);
...
exports.getResourcesFromHTMLFiles = function(gaiaPath, appPath) {
    var htmlPaths = exports.ls(appPath, true, /\.html$/).filter(isNotTestFile);
    return Promise.all(
      htmlPaths.map(
